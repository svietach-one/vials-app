 ...
 *   AC-15 Tapping an OBF result navigates to ManualProductForm with OBF prefill
...
// ── AC-15: Tapping OBF result navigates to ManualProductForm ─────────────────

describe('AC-15: tapping an OBF result navigates to ManualProductForm with OBF prefill', () => {
  it('should navigate to ManualProductForm with prefillOBFProduct when a result row is pressed', async () => {
    mockSearchProducts.mockResolvedValue({ products: [OBF_RESULT], failed: false });
    renderScreen();
    fireEvent.changeText(screen.getByTestId('hub-search-input'), 'vitamin c');
    act(() => jest.runAllTimers());
    ...
    expect(mockNavigate).toHaveBeenCalledWith('ManualProductForm', { prefillOBFProduct: OBF_RESULT });
  });
});
