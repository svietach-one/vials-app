...
> **Sync note (v1.1 → v1.2):** Adds four related, additive extensions to the routine
...
> SPF adequacy recommendation; (4) goal-coverage hints + a pregnancy/lactation safety
> guard (§4.2.3). Full acceptance criteria in `USER_STORIES.md` US-23–US-33. Full
> technical design and phased tasks in `IMPLEMENTATION_PLAN.md` Phases 8–11. The
> collision-matrix extension in §5.2 is a **research proposal pending clinical sign-off**,
> matching the existing caveat already applied to §5.3 — see §6. The pregnancy safety
> tiers (§4.2.3) rest on stronger consensus but also route through that same §6 sign-off.
...
#### 4.2.3 Goal-Coverage Hints & Pregnancy/Lactation Safety Guard (v1.2, NEW)

Two linked capabilities, designed together because safety outranks goal coverage when they
collide.

**Goal coverage.** Users optionally pick skincare goals (`SkinGoalType`, §5.1):
`wrinkles`, `pores`, `dryness`, `pigmentation`, `sebum_control`, `restoration`,
`protection`. Each goal maps to ingredient tags (or, for `protection`, to the `SPF` product
type / `spfValue` machinery from §4.2.1):

| Goal | Tags |
|---|---|
| Wrinkles | `RETI`, `PEPT`, `VIT_C` |
| Pores | `ACID`, `NIAC` |
| Dryness | `HYAL`, `GLYC`, `CERA`, `PANT` |
| Pigmentation | `VIT_C`, `AZA`, `NIAC`, `ACID`, `RETI`, `HYDRO` |
| Sebum control | `ACID`, `NIAC`, `BPO`, `ZPCA` |
| Restoration | `CERA`, `PANT`, `PEPT` |
| Protection | *(product type `SPF`)* |

After routine creation (and passively during Weekly Plan editing), if a stated goal has no
matching product scheduled, a gentle Cobalt, dismissible note surfaces — differentiated by
whether a matching product exists on the shelf but isn't scheduled ("you already have
something for this — it's just not in your routine") versus nothing on the shelf at all
(softest wording). Never a purchase push; Vials does not recommend the catalog.

**Pregnancy/lactation safety guard.** Driven by the existing `UserProfile.pregnantOrBreastfeeding`
flag (already surfaced as a `ListRow` in the Edit Skin Profile sheet with hint "We'll flag
retinoids and other restricted actives"). When set, restricted actives are handled by tier:

| Tier | Tags | Behavior |
|---|---|---|
| Restricted | `RETI`, `HYDRO` | Never offered as a goal solution. If already scheduled, soft advisory + mandatory "consider checking with your doctor or dermatologist." Never a hard block. |
| Caution | `ACID`, `BPO` | Not offered as the primary goal solution when a Safe-tier alternative exists; mild "some forms are used with caution during pregnancy — a professional can advise" if surfaced. |
| Safe alternative | `AZA`, `VIT_C`, `NIAC` | Actively preferred as the goal solution surfaced to pregnant/breastfeeding users. |
| Unaffected | `HYAL`, `GLYC`, `CERA`, `PANT`, `PEPT`, `ZPCA`, `SPF` | Normal behavior. |

**Safety outranks coverage:** if the only shelf product addressing a goal is a Restricted-
tier active, the app does NOT say "you have nothing for this goal" (which would nudge the
user toward the restricted product). It surfaces the pregnancy-safe alternatives for that
goal and points to a professional. Independently, any Restricted-tier active scheduled in
the routine — goal-related or not — triggers the standalone advisory that fulfills the
shipped `PREGNANCY_LABEL` hint's promise.

**Monetization:** this is a **free feature**, deliberately not gated behind Vials Pro. This
departs from `Vials_Business_Plan_v1_1.docx` §4 (which listed a pregnancy safety mode as
Pro) in favor of that same plan's §3.1 principle that safety warnings are never behind a
paywall. Pro may layer analytical depth on top later; the core guard stays free.

**Tone:** same non-absolute, non-diagnostic banned-phrase rules as §4.2.2 / US-26 — with
one addition unique to this feature: safety-conflict paths MUST include an explicit,
plain-language redirect to a professional. Full evidence base, tier rationale, and design
in the companion `GOAL_COVERAGE_AND_PREGNANCY_SAFETY.md`.
...
* **Core entities (minimum fields for store/schema design):**
  ...
  * `Profile`: `age, gender, phototype, skinIssues[], gamificationOn:boolean, individualDurationMonths: { [procedureType]: number }, skinConditions: SkinConditionType[], skinGoals: SkinGoalType[], pregnantOrBreastfeeding: boolean` — `skinConditions`, `skinGoals` are NEW (v1.2, default `[]`); `pregnantOrBreastfeeding` already exists on `UserProfile` and drives the §4.2.3 safety guard
  ...
  * `SkinGoalType` (NEW, v1.2): `'wrinkles' | 'pores' | 'dryness' | 'pigmentation' | 'sebum_control' | 'restoration' | 'protection'` — optional; drives the goal-coverage hints in §4.2.3.

> **Open item:** confirm `skinIssues[]`'s existing intended semantics don't already
> overlap with `skinConditions[]` or `skinGoals[]`. If `skinIssues[]` was in fact meant to
> hold goals, `skinGoals` should migrate/replace it rather than sit alongside. Kept
> separate until the product owner resolves this.
...
* `parseInciTags(inci: string)` targets conflict-grade groups: `RETI` (Retinoids), `ACID` (AHA/BHA/PHA), `VIT_C` (Ascorbic Acid), `PEPT` (Copper Peptides), and — **new in v1.2** — `BPO` (Benzoyl Peroxide), `AZA` (Azelaic Acid), `HYDRO` (Hydroquinone — a restriction-relevant active used by the pregnancy safety guard and the Pigmentation goal mapping in §4.2.3; not part of the collision matrix below).
...
* **(v1.2)** Confirm `skinIssues[]` vs. new `skinConditions[]` / `skinGoals[]` field
  overlap with the product owner (§5.1).
* **(v1.2)** The pregnancy/lactation restriction tiers in §4.2.3 need clinical sign-off in
  the same combined pass. The `RETI`/`HYDRO` restrictions are high-consensus; the
  `ACID`/`BPO` caution-tier placement (over-cautious by design, since the app can't read
  concentration) and the exact professional-redirect copy specifically need reviewer
  confirmation. Also confirm the safety-guard copy is non-diagnostic and appropriately
  disclaims that it's not medical advice.