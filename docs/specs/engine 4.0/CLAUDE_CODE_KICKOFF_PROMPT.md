# Kickoff Prompt — Vials Routine Engine v1.2 (Phases 8–11)
...
I need you to implement four additive extensions to the Vials routine/conflict engine,
already fully specced. Read before touching any code:

1. `docs/PRD_Spec.md` v1.2 — read all of it, but especially §4.2.1, §4.2.2, §4.2.3, §5.1,
   §5.2 (Active Ingredient Density Insights subsection), and §6.
2. `docs/USER_STORIES.md` — US-23 through US-33 (acceptance criteria, source of truth for
   "done").
3. `docs/IMPLEMENTATION_PLAN.md` — Phase 8, Phase 9, Phase 10, Phase 11 (task breakdown,
   file paths, dependency notes — there are no new npm/expo packages needed for any of this).
   Also read `docs/GOAL_COVERAGE_AND_PREGNANCY_SAFETY.md` for the Phase 11 evidence base
   and full design.
...
- **Phase 11 is safety-critical — treat it with extra care.** The pregnancy/lactation
  guard (`US-32`, `US-33`) must fail safe: when in doubt, it surfaces the professional-
  redirect rather than staying silent, and it must NEVER report a goal as "uncovered" when
  the only shelf match is a restricted active (that would nudge a pregnant user toward a
  retinoid — the exact thing the feature exists to prevent). Every safety path MUST emit
  the explicit "consider checking with your doctor or dermatologist" line. It must also
  never diagnose or use absolute language ("unsafe," "cannot") — frame restricted actives
  as "usually set aside during pregnancy." The `pregnantOrBreastfeeding` field already
  exists on `UserProfile` and the Edit Skin Profile sheet already promises "We'll flag
  retinoids and other restricted actives" — your engine must actually deliver on that
  promise (flag at least `RETI` and `HYDRO`), and there's a test for hint-vs-behavior
  agreement. This feature is intentionally FREE (not Pro-gated) — do not add any
  entitlement/paywall check around it.
...
fully independent) → Phase 11 (goal coverage + pregnancy safety guard, depends on the
`HYDRO` tag and reuses the `ConditionSelector`-style picker from Phase 8.2). Phases 10 and
11 are independent of 8/9 and each other's engines and can be split across people if
useful. Within each phase, do the `.0`/`.1` data-model-and-engine steps first and get
their unit tests green before touching any UI component.
...
A third known open item: `skinIssues[]` may overlap with the new `skinGoals[]` field too
(`PRD_Spec.md` §5.1) — same "needs product-owner input" caveat, don't silently migrate it.
