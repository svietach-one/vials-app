...
## Phase 11 — Goal-Coverage Hints + Pregnancy/Lactation Safety Guard

**Goal:** Tell users, gently, when their routine doesn't address a stated skincare goal,
and guard pregnant/breastfeeding users against restricted actives — with safety
outranking goal coverage whenever they collide. Free feature (not Pro-gated). Full
evidence base and design: `GOAL_COVERAGE_AND_PREGNANCY_SAFETY.md`.

**Reference:** `PRD_Spec.md` §4.2.3, §5.1, §5.2 (`HYDRO` tag); `USER_STORIES.md`
US-30–US-33.

### Phase 11.0 — Data Model
- Add `SkinGoalType` and `skinGoals: SkinGoalType[]` to `src/types/index.ts` + profile store
- Add `HYDRO` to `ActiveIngredientKey`; add its `parseInciTags` regex
- Confirm `pregnantOrBreastfeeding` is already present on `UserProfile` (it is — surfaced
  in the Edit Skin Profile sheet via `PREGNANCY_LABEL`); wire engine reads to it
...
- Unit tests: each goal's coverage states (covered / on-shelf-not-scheduled / not-on-shelf);
  pregnancy safety-redirect (only-restricted-on-shelf → `safety_redirect`, NOT "nothing
  found"); safe-alternative preference; restricted-in-routine standalone flag;
  `skinGoals === []` and `pregnantOrBreastfeeding === false` no-ops; hint-text-vs-behavior
  agreement (US-32)

### Phase 11.2 — UI
- Onboarding: optional goal-selection step (reuse the `ConditionSelector`-style
  multi-select from Phase 8.2)
- `SkinProfileEditor`: goals editable; the `pregnantOrBreastfeeding` `ListRow` already
  exists — no new row needed
- Today screen: Cobalt goal-coverage banner + Amber pregnancy-safety advisory, both
  dismissible via `settingsStore.dismissedBanners`; mandatory professional-redirect line on
  safety paths
- Optional: passive note in the Weekly Plan creation flow

### Phase 11.3 — Compliance
- Extend the banned-phrase lint (US-26) to goal/safety templates
- **Clinical sign-off:** pregnancy tier table added to the `PRD_Spec.md` §6 combined review
  pass — `RETI`/`HYDRO` are high-consensus; `ACID`/`BPO` caution placement and the
  professional-redirect copy need explicit reviewer confirmation
- Verify the shipped `PREGNANCY_LABEL` hint ("We'll flag retinoids and other restricted
  actives") now matches real behavior — engine must flag at least `RETI` and `HYDRO`
...
Placeholder entries only — no design work done yet, listed here so they aren't lost
between planning sessions. Each will get its own PRD addendum / user stories / tech
design pass, same process as Phases 8–11, when picked up.

### Phase 12 (candidate) — Proactive Shelf Optimization

**Goal (one-liner, undesigned):** Recommend how to use products the user already owns
more effectively — not a conflict/risk warning about what's already in the routine, and
broader than Phase 11's binary "is this goal covered at all" check: guidance on using the
shelf *optimally* (ordering, frequency, pairing). Explicitly a different kind of system
from Phases 8–11: those are reactive or binary-coverage; this would be fully
recommendation-oriented.

Not scoped. Likely reuses existing data already captured for other purposes —
`skinGoals[]`/`skinConditions[]`, `active_ingredient_keys`, and the catalog biomarker
tags (`Soothing`/`Actives`/`Hydration`) — as inputs, but needs its own optimization logic
beyond Phase 11's coverage check. Flagged as a materially larger feature — expect its own
sizing pass before committing to a phase breakdown.
