...
>
> **Sync note (v1.2):** Reconciled with `PRD_Spec.md` v1.2. Adds `ConditionSelector`
> (onboarding + `SkinProfileEditor`), a `GoalSelector` (onboarding + `SkinProfileEditor`),
> extends `ConflictWarningInline` with condition-risk and active-ingredient-density rows,
> extends `ClinicalRestrictionsBlock` and `ProcedureLifespanCard`/`FadingInteractivePrompt`
> with condition-aware recovery caution, extends `SeasonalNoticeBanner` with the SPF
> adequacy recommendation, and adds a Today-screen goal-coverage banner + pregnancy-safety
> advisory. See `PRD_Spec.md` §4.2.1–4.2.3 for full behavior and `USER_STORIES.md`
> US-23–US-33 for acceptance criteria.
...
* **`ConditionSelector` (v1.2, `US-23`):** Optional step after `PhototypeSelector`.
  Multi-select (0–3) for Eczema/Atopic Dermatitis, Seborrheic Dermatitis, Rosacea, each
  with a plain-language, non-clinical description and a persistent "not a diagnosis"
  disclaimer. Skippable, same pattern as `FirstProductScreen`'s skip path below. Also
  reachable post-onboarding via `SkinProfileEditor` (Tab 4). Writes to
  `profileStore.skinConditions`.
* **`GoalSelector` (v1.2, `US-30`):** Optional multi-select for skincare goals (Wrinkles,
  Pores, Dryness, Pigmentation, Sebum control, Restoration, Protection). Same skippable,
  plain-language pattern as `ConditionSelector`. Writes to `profileStore.skinGoals`; drives
  the Today-screen goal-coverage hints (`US-31`) and, together with the
  `pregnantOrBreastfeeding` flag, the safety-outranks-coverage logic (`US-33`). Also
  editable via `SkinProfileEditor` (Tab 4).
...
* **`ClinicalRestrictionsBlock`:** Conditional card (`#FAF9F6`). If an active procedure is in `Rehab` status, displays restriction icons (No gym, No sun) rendered with **Rich Cabernet** highlights, alongside safe indicators using **Deep Bottle Green**. **(v1.2)** During Rehab for an aggressive procedure type, appends one Amber advisory line if the user has eczema and/or rosacea selected — copy-only, does not alter the rehab timeline.
* **`SeasonalNoticeBanner`:** Flat, dismissible container providing adaptive climate advice during seasonal cut-off dates. **(v1.2)** Also surfaces the SPF adequacy recommendation (Cobalt, dismissible per-season) when the user's phototype is Light/Fair, it's summer, and their scheduled sunscreen's `spfValue` is below 30. See `PRD_Spec.md` §4.2.1.
* **Goal-Coverage & Safety Banners (v1.2, `US-31`/`US-32`/`US-33`):** After routine creation, the Today screen surfaces (via the same dismissible banner infrastructure) two additive, non-blocking notices: a **Cobalt** goal-coverage hint when a stated `skinGoal` isn't addressed by the routine (differentiated by whether a matching product is on the shelf but unscheduled vs. absent entirely), and — only when `pregnantOrBreastfeeding` is set — an **Amber** pregnancy-safety advisory for any Restricted-tier active (`RETI`, `HYDRO`) scheduled, which MUST include an explicit "consider checking with your doctor or dermatologist" redirect. When a goal's only shelf match is a restricted active, the goal hint is replaced by a safety-redirect that names pregnancy-safe alternatives rather than reporting the goal as uncovered. Neither ever blocks or uses Cabernet. See `PRD_Spec.md` §4.2.3.
...
* **`ConflictWarningInline`:** Mounts dynamically at the bottom of the column if ingredients overlap on the exact same day, per the full pairwise collision table in `conflictEngine.ts`, highlighted with a **Rich Amber** alert badge. **(v1.2)** Renders three additional, visually distinct row kinds: `condition-advisory` (Amber, single-ingredient risk relevant to a selected skin condition — never uses the word "conflict"), `density-insight` (**Cobalt** — same active ingredient repeated across ≥2 products in one AM/PM period, mild tags only, informational not cautionary), and `density-warning` (Amber — same repetition, but for `RETI`/`ACID`/`BPO`). None of the four row kinds ever block saving. See `PRD_Spec.md` §4.2.2 and §5.2.
...
* **`SkinProfileEditor`:** Input form sheets updating age, gender metrics, and skin issues. Houses the 3 phototype card selectors (see accessibility note in Section 1). **(v1.2)** Also houses the same `ConditionSelector` and `GoalSelector` used in onboarding, both editable at any time post-onboarding, plus the existing `pregnantOrBreastfeeding` `ListRow` (`PREGNANCY_LABEL`, hint "We'll flag retinoids and other restricted actives") that drives the §4.2.3 safety guard.
