...
> (onboarding + `SkinProfileEditor`), extends `ConflictWarningInline` with condition-risk
> and active-ingredient-density rows, extends `ClinicalRestrictionsBlock` and
> `ProcedureLifespanCard`/`FadingInteractivePrompt` with condition-aware recovery caution,
> and extends `SeasonalNoticeBanner` with the SPF adequacy recommendation. See
> `PRD_Spec.md` §4.2.1–4.2.2 for full behavior and `USER_STORIES.md` US-23–US-29 for
> acceptance criteria.
...
* **`SkinProfileEditor`:** Input form sheets updating age, gender metrics, and skin issues. Houses the 3 phototype card selectors (see accessibility note in Section 1). **(v1.2)** Also houses the same `ConditionSelector` used in onboarding, editable at any time post-onboarding.
