...
CosmeticsProduct {
  ...
  spf_value            number | null        — NEW (2026-07-25): SPF value, only meaningful when product_type = 'SPF'; powers the phototype/season-aware SPF adequacy recommendation (see Vials app-repo docs, PRD_Spec.md §4.2.1)
...
| `spf_value` | integer, nullable, expected range roughly 2–100 | Only meaningful when `product_type = 'SPF'`; app-side check treats missing values as "unknown," never as a worst-case assumption |
