...
CREATE TABLE products (
  ...
  image_url              text,
  spf_value              int,                 -- NEW (2026-07-25); only meaningful when product_type = 'SPF'
  rating                 numeric(3,1),
  rating_count           int NOT NULL DEFAULT 0,
  source                 text NOT NULL DEFAULT 'community',
                           -- 'vials_seed' | 'obf_import' | 'community' | 'user_local'
                           -- NOTE: Vials_Business_Plan_v1_1.docx §2.2 states the source
                           -- enum was updated to drop 'obf_import' project-wide (no OBF
                           -- data is imported). This file wasn't updated to match at the
                           -- time of this pass — flagging, not fixing, since it's outside
                           -- this task's scope.
