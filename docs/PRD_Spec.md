...
* ~~Confirm the Vials API moderation queue workflow (admin review of `pending` crowdsourced suggestions) is defined and staffed before launch.~~ **Retired 2026-07-19 — moot.** There is no Vials API to moderate (see the sync note at the top of this document and §4.3: the submission pipeline is "not built"). A moderation workflow cannot be defined or staffed for a service that does not exist. US-3 (community contribution) is **deferred — blocked on no backend**; community contribution UI is gated off behind `COMMUNITY_CONTRIBUTION_ENABLED` (`src/constants/featureFlags.ts`) so nothing tells a user their submission was saved. This item returns — together with the moderation question — if and when the API is greenlit: see `docs/tasks/product-images/ROADMAP-vials-api-contribution.md`.